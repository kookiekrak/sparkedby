...
def get_secrets():
    ...
    if environment != 'production':
        ...
        logger.debug(f"Environment variables available: {[k for k, v in secrets_dict.items() if v is not None]}")
        return secrets_dict
        
    try:
        logger.info("Production environment detected, loading secrets from AWS")
        ...
        logger.debug("Attempting to get secret 'whisperworker' from Secrets Manager")
        ...
        if 'API_URL' in secrets:
            logger.debug("Found API_URL in secrets")
            if not secrets['API_URL']:
                logger.warning("API_URL in secrets is empty or null")
            else:
                logger.debug("API_URL from secrets appears to be valid")
        else:
            logger.warning("API_URL key not found in secrets")
            
        return secrets
    except ClientError as e:
        logger.error(f"Failed to get secrets in production: {e}")
...
def process_message(message):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Initial API_URL from secrets: {api_url}")
            
            if not api_url:
                api_url = os.getenv('API_URL', 'http://localhost:3000')
                logger.warning(f"API_URL not found in secrets, using fallback: {api_url}")
            else:
                logger.debug(f"Using API_URL from secrets: {api_url}")
            ...
            try:
                ...
                if not response.ok:
                    ...
                    raise Exception(f"Failed to update container text: {response.status_code}")

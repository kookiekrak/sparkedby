...
def get_secrets():
    """Get secrets from AWS Secrets Manager by default, fallback to environment variables in development"""
    
    # Default to production behavior - use AWS Secrets Manager
    try:
        ...
        if os.getenv('ENVIRONMENT') == 'development':
            logger.info("Development mode detected, using environment variables")
            return get_development_secrets()
        ...
        # Only fallback to environment variables in development
        if os.getenv('ENVIRONMENT') == 'development':
            logger.info("Falling back to environment variables in development mode")
            return get_development_secrets()
        raise  # In production, we want to fail if we can't get secrets

def get_development_secrets():
    """Get secrets from environment variables for development mode"""

...
const sqsClient = new client_sqs_1.SQSClient({
    ...
    credentials: (0, credential_providers_1.fromEnv)() // This will look for AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY
});
const getQueueUrl = () => {
    ...
    if (environment === 'production') {
        if (!prodQueue)
            throw new Error('Missing WHISPER_QUEUE_URL_PROD configuration');
        return prodQueue;
    }
    if (!devQueue)
        throw new Error('Missing WHISPER_QUEUE_URL_DEV configuration');
    return devQueue;
};
const handleProcessAudio = async (req, res) => {
    ...
    if (!visitId)
        missingParams.push('visitId');
    if (chunkId === undefined || chunkId === null)
        missingParams.push('chunkId');
    if (isFinal === undefined)
        missingParams.push('isFinal');
...
exports.processAudio = (0, handlers_1.wrapAuthenticatedHandler)(handleProcessAudio);

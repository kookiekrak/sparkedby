...
// dotenv is already loaded by env-loader, but we keep this for backward compatibility
dotenv.config();
...
// Enhanced error handling for the update-container-text endpoint
app.post("/update-container-text", authenticateApiToken, express.json(), (req, res, next) => {
  console.log('[API] Received update-container-text request', {
  ...
  if (!req.body || Object.keys(req.body).length === 0) {
    console.error('[API] Empty or invalid JSON body in update-container-text request');
    res.status(400).json({ error: 'Invalid or empty request body' });
    return; // Return void instead of the response
  }
  
  next();
}, wrapAuthenticatedHandler(handleUpdateContainerText));
...
app.post('/notes/:noteId/regenerate-soap', authenticateUser, wrapAuthenticatedHandler(handleSoapNoteRegeneration));
app.post('/notes/:noteId/regenerate-template', authenticateUser, wrapAuthenticatedHandler(handleTemplateNoteRegeneration));
